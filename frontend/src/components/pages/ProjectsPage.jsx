...
import { ShineCard, ShineCardContent, ShineCardDescription, ShineCardHeader, ShineCardTitle } from "../magicui/shine-card";
...
const projectsData = [
  ...
  },
  {
    id: 'watt-watt',
    title: 'WattWatt.nl',
    description: 'Developed WattWatt.nl website based on Figma designs with form integrations for backend functionality.',
    detailedDescription: 'Developed WattWatt.nl website based on Figma designs. Coded with Next.js and Tailwind, including form integrations with Zapier and Google Sheets for backend functionality.',
  ...
  },
  {
    id: 'daccs',
    title: 'Daccs',
    description: 'UI Development and full-stack implementation with MySQL database integration on AWS.',
    detailedDescription: 'UI Development and full-stack implementation with MySQL database integration. Built on AWS infrastructure for scalability and reliability.',
  ...
  },
  {
    id: 'perspectivity',
    title: 'Perspectivity',
    description: 'Full software lifecycle implementation with Python and NLP capabilities on cloud infrastructure.',
    detailedDescription: 'Full software lifecycle implementation with Python and NLP capabilities. Developed cloud infrastructure and extensive data processing pipelines.',
...
const ProjectsPage = () => {
      ...
      entries.forEach(entry => {
        ...
        if (index !== -1) {
          if (entry.isIntersecting) {
            ...
            const nextIndex = activeProjectIndex < index ? index : activeProjectIndex;
            const prevIndex = activeProjectIndex > index ? index : activeProjectIndex;
  ...
  useEffect(() => {
    ...
    const nextProject = nextProjectIndex !== null ? projectsData[nextProjectIndex] : null;
    
    projectSections.forEach((section, index) => {
      if (index === activeProjectIndex) {
        if (nextProjectIndex !== null) {
          // Create a smooth transition between colors
          const blendedFrom = blendColors(currentProject.gradientFrom, nextProject.gradientFrom, scrollProgress);
          const blendedTo = blendColors(currentProject.gradientTo, nextProject.gradientTo, scrollProgress);
  ...
  if (projectRefs.current.length !== projectsData.length) {
    projectRefs.current = Array(projectsData.length).fill().map((_, i) => projectRefs.current[i] || null);
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      >
        <div className="w-full px-4 md:px-6 max-w-7xl mx-auto z-10">
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">
              <GradientText 
                gradient={theme === 'dark' ? "from-white via-gray-300 to-white" : "from-indigo-600 via-purple-600 to-pink-600"}
                animate={true}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {projectsData.map((project, index) => (
              <ShineCard 
                key={index}
                className={`group bg-white/5 backdrop-blur-sm border-white/10 h-60 cursor-pointer transform transition-all duration-500 hover:scale-105 ${
              ...
              >
                <ShineCardHeader className="mb-4">
                  <ShineCardTitle className={theme === 'light' ? "text-primary" : "text-white"}>
                    {project.title}
                  </ShineCardTitle>
                  <ShineCardDescription className={theme === 'light' ? "text-muted-foreground" : "text-white/70"}>
                    {project.description}
          ...
          <div className="text-center animate-bounce">
            ...
            <p className={theme === 'light' ? "text-gray-500 mt-2" : "text-white/50 mt-2"}>
            ...
            <div className="w-full max-w-7xl mx-auto px-4 md:px-6 relative z-10">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div className="transform transition-all duration-1000">
                  ...
                  <p className="text-white/90 text-lg mb-8 font-medium">{project.detailedDescription}</p>
                  <a 
                    href={project.link} 
                    target="_blank" 
                  ...
                  >
                    <Button 
                      className="bg-white text-black hover:bg-white/90 transition-colors duration-300 transform hover:scale-105 font-semibold"
                    >
                      Visit Project
                    </Button>
                  </a>
